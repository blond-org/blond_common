...
#             self.imped_calc = self._imped_calc_cpp
        if method == 'python':
            ...
            #WrongCalcError
            raise RuntimeError(
                'method for impedance calculation in Resonator object ' +
                'not recognized')
...
class TravelingWaveCavity(_ImpedanceObject):
    ...
    both wake and impedance. The resonance modes (and the corresponding R and
    a) can be inputed as a list in case of several modes.
            ...
            Zplus = self.R_S[i] * (
                (np.sin(
                    ...
                    (self.a_factor[i] / 2 * (
                        ...
                        self.frequency_R[i])))**2 - 2j*(
                            self.a_factor[i] *
                            (self.frequency_array - self.frequency_R[i]) -
                            np.sin(self.a_factor[i] * (
                                self.frequency_array -
                                self.frequency_R[i]))) / (
                                    self.a_factor[i] *
                                    (self.frequency_array -
...
class ResistiveWall(_ImpedanceObject):
    ...
    def __init__(self, pipe_radius, pipe_length,
